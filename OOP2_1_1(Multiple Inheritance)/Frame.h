...
class Frame
{
    public:
        ...
        Frame(char horizontal, char vertical, char corner)
            : _horizontal(horizontal), _vertical(vertical), _corner(corner),
            _has_ruler(true) {}

        explicit Frame(char vertical)
        ...
        Align alignment(unsigned i) const { return _alignment[i]; }
    ...
    private:
        ...
        std::map<unsigned, Align> mutable _alignment;
        ...
        void setupAlignment() const {
            ...
            for (unsigned i = 0; i < columns(); ++i) {
                if (_alignment.find(i) == _alignment.end()) {
                    _alignment[i] = Align::LEFT;
                }
            }
        }
    };

    inline std::ostream& operator<<(std::ostream& stream, const Frame& table) {
        if (table.rows().empty()) {
            return stream;
        }
        table.setup();
        ...
        for (auto rowIterator = table.rows().begin();
            ...
            Frame::Row const& row = *rowIterator;
            stream << table.vertical();
            for (unsigned i = 0; i < row.size(); ++i) {
                auto alignment = table.alignment(i) == Frame::Align::LEFT
                    ? std::left
                    : std::right;
